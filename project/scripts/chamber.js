...
function renderDirectory(filteredData = directoryData) {
    ...
    filteredData.forEach(biz => { // Use filteredData
        ...
        card.innerHTML = `
            <img src="${biz.image}" alt="${biz.name}" class="biz-image">
            <div class="biz-details">
                <h3>${biz.name}</h3>
                <p><strong>Address:</strong> ${biz.address}</p>
                <p><strong>Phone:</strong> ${biz.phone}</p>
                <p><strong>Membership Level:</strong> ${biz.level}</p>
                <p><strong>Dedicated Since:</strong> ${biz.dedicated}</p>
                <p><strong>Area:</strong> ${biz.area} sqm</p>
                <p><a href="${biz.website}" target="_blank" class="website-link">Visit Website</a></p>
            </div>
        `;
